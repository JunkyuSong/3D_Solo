...
HRESULT CTimer_Manager::Set_TimeSpeed(const _tchar * pTimerTag, const _float & _fTimeSpeed)
{
	CTimer*		pTimer = Find_Timer(pTimerTag);
	if (nullptr == pTimer)
		return E_FAIL;

	pTimer->Set_TimeSpeed(_fTimeSpeed);

	return S_OK;
}

_float CTimer_Manager::Get_TimeSpeed(const _tchar * pTimerTag)
{
	CTimer*		pTimer = Find_Timer(pTimerTag);
	if (nullptr == pTimer)
		return -1.f;

	return pTimer->Get_TimeSpeed();
}
