...
HRESULT CDirLight::Initialize(const DIRLIGHTDESC & LightDesc)
{
	...
	_LightDirInverseMatrix.r[3] = XMLoadFloat4(&LightDesc.vDirection) * -10.f;
...
{
	{
		...
		_vector _vPos = static_cast<CTransform*>(_pGameInstance->Get_Player()->Get_ComponentPtr(TEXT("Com_Transform")))->Get_State(CTransform::STATE_POSITION);
		_LightDirInverseMatrix.r[3] = XMVectorSetW( XMLoadFloat4(&m_LightDesc.vDirection) * -5.f + _vPos, 1.f);
